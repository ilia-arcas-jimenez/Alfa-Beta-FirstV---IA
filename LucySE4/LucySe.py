...
if scoAvg == 0:
    ...
    with open("F:/desktop/importante/PROYECTOS DE PROGRAMACION/Python/sentimientos/LucySE4/contenidoPositivo.json", ) as archivo2:
        datosPositivos = json.load(archivo2)
else:
    with open("F:/desktop/importante/PROYECTOS DE PROGRAMACION/Python/sentimientos/LucySE4/contenidoNegativo.json", ) as archivo3:
        datosNegativos = json.load(archivo3)
palabras=[]
tags=[]
auxX=[]
auxY=[]
...
def Lucy():
    ...
    if scoAvg == 0:
        ...
        with open("F:/desktop/importante/PROYECTOS DE PROGRAMACION/Python/sentimientos/LucySE4/contenidoPositivo.json",) as archivo2:
            datosPositivos = json.load(archivo3)
        corchete = [0 for _ in range(len(palabras))]
        entradaProcesada = nltk.word_tokenize(entrada)
        entradaProcesada = [stemmer.stem(palabra.lower()) for palabra in entradaProcesada]
        for palabraInidvidual in entradaProcesada:
